...
class WxPayApi
{
     ...
     * @param WxPayExtract $inputObj
     * @param int $timeOut
     * @throws WxPayException
     * @return 成功时返回，其他抛异常
     */
    public static function Extract($inputObj, $timeOut = 6){
        ...
        $result = self::postXmlCurl($xml, $url, true, $timeOut);
    ...
    public static function redPack($inputObj, $timeOut = 6){
        ...
        if(!$inputObj->IsSend_nameSet()) {
            throw new WxPayException("缺少发红包接口必填参数send_name！");
        }else if(!$inputObj->IsRe_openidSet()){
            throw new WxPayException("缺少发红包接口必填参数re_openid！");
        }else if(!$inputObj->IsTotal_amountSet()) {
            throw new WxPayException("缺少发红包接口必填参数total_amount！");
        }else if(!$inputObj->IsTotal_numSet()) {
            throw new WxPayException("缺少发红包接口必填参数total_num！");
        }else if(!$inputObj->IsWishingSet()) {
            throw new WxPayException("缺少发红包接口必填参数wishing！");
        }else if(!$inputObj->IsAct_nameSet()) {
            throw new WxPayException("缺少发红包接口必填参数act_name！");
        }else if(!$inputObj->IsRemarkSet()) {
            throw new WxPayException("缺少发红包接口必填参数remark！");
        }else if(!$inputObj->IsMch_BillnoSet()) {
            throw new WxPayException("缺少发红包接口必填参数mch_billno！");
        }else { }
